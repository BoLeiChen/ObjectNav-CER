...
from utils.utils_csr import my_shuffle_evaluate
...
class MocoV2(pl.LightningModule):
    ...
    def _step_helper(self, batch, batch_idx, is_train):
        ...
        q_dict, k_dict = batch
        img_q = torch.cat(
            (q_dict['image'], q_dict['mask_1'].unsqueeze(1), q_dict['mask_2'].unsqueeze(1)), 1)
        img_k = torch.cat(
            (k_dict['image'], k_dict['mask_1'].unsqueeze(1), k_dict['mask_2'].unsqueeze(1)), 1)
